...
dataset_file_prefix = "dataset/Pet"
head_class = [3, 3, 10, 10]
tail_count = [3,   5,   3,   5,]


def get_sorted_class_list():
    class_count = []
    for item in os.listdir(dataset_file_prefix + "/Pet_byclass"):
        class_count.append([item, len(os.listdir(dataset_file_prefix + "/Pet_byclass/" + item))])
...
def build_dataset(head_class_count, tail_count, class_count, batch_size):
    ...
    for head_id, _ in class_count[0:head_class_count]:
        path = dataset_file_prefix + "/Pet_byclass/" + str(head_id)
    ...
    np.save(dataset_file_prefix + "/Pet_" + f"{head_class_count}_{tail_count}" + "/feature", output_feat)
    np.save(dataset_file_prefix + "/Pet_" + f"{head_class_count}_{tail_count}" + "/label", output_label)


if __name__ == '__main__':
    try:
        for i in range(len(head_class)):
            os.mkdir(dataset_file_prefix + "/Pet_" + str(head_class[i]) + "_" + str(tail_count[i]))
    except:
        pass
